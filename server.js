...
const jobs=new Map();
function schedule(m){
  if(jobs.has(m.id)){clearInterval(jobs.get(m.id));jobs.delete(m.id);}
  if(!m.active)return;
  const ms=Math.max(5,m.interval)*1000;
  jobs.set(m.id,setInterval(()=>runCheck(m),ms));
}
